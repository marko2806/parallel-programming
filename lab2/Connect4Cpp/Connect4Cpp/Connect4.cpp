...
void odigraj_potez(Potez* tretnutni_potez, Potez* novi_potez, int col) {
    ...
    novi_potez->made_by_player = tretnutni_potez->made_by_player ^ -2;
    ...
    std::memcpy(novi_potez->state, tretnutni_potez->state, sizeof(int) * NUM_OF_ROWS * NUM_OF_COLS);
    int red = 0;
    ...
    if (red > 0) {
        novi_potez->state[red - 1][col] = novi_potez->made_by_player;
    }
}
