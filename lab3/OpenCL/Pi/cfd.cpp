...
{
	int printfreq=1; //output frequency
	double error, bnorm;
	...
	list_value(psi, 0.0, (m + 2) * (n + 2));
	// Test
	/*for (int i = 0; i < m + 2; i++) {
		for (int j = 0; j < n + 2; j++) {
			if (psi[i * (m + 2) + j] != 0) {
				printf("Razlicito od 0, %f %d %d\n", psi[i * (m + 2) + j], i, j);
				exit(1);
			}
		}
	}*/
	...
	for(iter=1;iter<=numiter;iter++) {
		...
		/*double* psi_test = (double*)malloc(sizeof(double) * (m + 2) * (n + 2));
		for (int i = 0; i < m + 2; i++) {
			for (int j = 0; j < n + 2; j++) {
				psi_test[i * (m + 2) + j] = psi[i * (m + 2) + j];
			}
		}
		...
		*/
		copy_list(psi, psitmp, (m + 2) * (n + 2));

		/*for (int i = 0; i < m + 2; i++) {
			for (int j = 0; j < n + 2; j++) {
				if (psi_test[i * (m + 2) + j] != psi[i * (m + 2) + j]) {
					printf("Razlicto %f, %f, %d %d\n", psi[i * (m + 2) + j], psi_test[i * (m + 2) + j], i, j);
					exit(1);
				}
			}
		}
